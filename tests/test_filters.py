...
def test_extract_filters_strips_json_code_fence():
    ...
    assert extract_filters("...", llm, _TODAY) == {"city": "Paris", "department": "75"}


def test_extract_filters_recovers_json_from_surrounding_prose():
    llm = _FakeLLM('Bien sûr : {"department": "93"} voilà.')
    assert extract_filters("...", llm, _TODAY) == {"department": "93"}


def test_extract_filters_drops_null_and_empty_values():
    llm = _FakeLLM('{"city": "", "department": "75", "date_from": null}')
    assert extract_filters("...", llm, _TODAY) == {"department": "75"}
...
def test_department_predicate_matches_exactly():
    predicate = build_metadata_filter({"department": "75"})
    assert predicate({"department": "75"}) is True
    assert predicate({"department": "93"}) is False


def test_city_predicate_is_case_insensitive():
    predicate = build_metadata_filter({"city": "Paris"})
    assert predicate({"city": "paris"}) is True
    assert predicate({"city": "Lyon"}) is False
...
def test_combined_filters_require_all_conditions():
    predicate = build_metadata_filter({"city": "Paris", "department": "75"})
    assert predicate({"city": "Paris", "department": "75"}) is True
    assert predicate({"city": "Paris", "department": "93"}) is False
